...
module.exports = class warnCommand extends Command {
    ...
    async run(message, args) {
        ...
        const punchannel = message.guild.channels.cache.find(ch => ch.name === "avisos-logs")
        if(!punchannel) {
            message.reply("Não foi possível encontrar o canal de punições, desde então, estarei enviando a embed aqui.").then(msg => msg.delete({ timeout: 5000 }))
